...
class TestRelationsAPISupport:
    ...
    @pytest.mark.integration
    def test_api_entry_creation_with_relations(self, dict_service_with_db: DictionaryService):
        ...
        result_id = dict_service_with_db.create_entry(entry)
        assert result_id is not None
        assert len(entry.relations) == 2
        assert entry.relations[0].type == "synonym"

    @pytest.mark.integration
    def test_api_entry_update_with_relations(self, dict_service_with_db: DictionaryService):
        """Test updating entry relations via API."""
        # Create entry
        entry = Entry(lexical_unit={"en": "modify"}, senses=[{"id": "sense1", "glosses": {"en": "test definition"}}])
        ...
        # Verify update
        updated_entry = dict_service_with_db.get_entry(entry_id)
...
class TestRelationsRangesIntegration:
    ...
    def test_relation_types_from_ranges(self, app: Flask):
        ...
        with app.app_context():
            ...
            # Should have lexical-relation range with default values
            assert 'lexical-relation' in ranges
            relation_types = ranges['lexical-relation']['values']
...
class TestRelationsLifecycle:
    ...
    @pytest.mark.integration
    def test_add_relation_to_existing_entry(self, dict_service_with_db: DictionaryService):
        """Test adding a relation to an existing entry."""
        # Create base entry
        entry = Entry(lexical_unit={"en": "base"}, senses=[{"id": "sense1", "glosses": {"en": "test definition"}}])
        entry_id = dict_service_with_db.create_entry(entry)
        # Get the entry from database to ensure we have the correct object
        entry = dict_service_with_db.get_entry(entry_id)
        # Add relation
        ...
        dict_service_with_db.update_entry(entry)
        # Verify
        updated = dict_service_with_db.get_entry(entry_id)
        assert len(updated.relations) == 1
        assert updated.relations[0].type == "synonym"

    @pytest.mark.integration
    def test_remove_relation_from_entry(self, dict_service_with_db: DictionaryService):
    ...
    @pytest.mark.integration
    def test_modify_relation_in_entry(self, dict_service_with_db: DictionaryService):
