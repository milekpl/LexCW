...
class TestDictionaryServiceFilteringSorting:
    ...
    def test_list_entries_with_sort_order_asc(self) -> None:
        ...
        with patch.dict("os.environ", {"TESTING": "true"}):
            with patch.object(DictionaryService, "_count_entries_with_filter", return_value=2):
                service = DictionaryService(mock_connector)
    ...
    def test_list_entries_with_filter_text(self) -> None:
        ...
        with patch.dict("os.environ", {"TESTING": "true"}):
            with patch.object(DictionaryService, "_count_entries_with_filter", return_value=1):
                service = DictionaryService(mock_connector)
                with patch("app.parsers.lift_parser.LIFTParser.parse_string") as mock_parse:
                    mock_parse.return_value = [
                        Entry(
