...
def test_merge_singular_plural_pairs():
    ...
    service.ranges_parser.parse_string = Mock(return_value={
        ...
        'variant-types': {'id': 'variant-types', 'values': [{'id': 'c'}], 'labels': {}, 'descriptions': {}},
    })
    ...
    assert ('variant-type' in keys) ^ ('variant-types' in keys)  # exactly one present

    # The remaining should have all unique values
    remaining = merged.get('variant-type') or merged.get('variant-types')
