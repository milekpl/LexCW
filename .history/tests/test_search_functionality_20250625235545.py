...
class TestSearchFunctionality(unittest.TestCase):
    ...
    def test_basic_search(self):
        ...
        try:
            ...
            entries, total = self.service.search_entries("test", limit=limit)
            logger.info("Using service method: Found %d entries for 'test' (total=%d)", len(entries), total)
            self.assertLessEqual(len(entries), limit, "Number of entries from service method should not exceed the limit")
