...
class TestLIFTParser:
    ...
    def test_generate_lift_string(self):
        ...
        assert gloss_elem is not None
        assert gloss_elem.text == "przykład"
        
        # Verify definition
        definition = sense_elem.find(".//lift:definition/lift:form[@lang='en']/lift:text", namespaces)
        assert definition is not None
        assert definition.text == "a thing characteristic of its kind"
          # Verify example
        example_elem = sense_elem.find(".//lift:example", namespaces)
        assert example_elem is not None
        assert example_elem.get("id") == "example1"
        
        # Verify example translation (note: in the generated XML, example only has translation, not form)
        example_trans = example_elem.find(".//lift:translation/lift:form[@lang='pl']/lift:text", namespaces)
