...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print(f'Usage: python -m scripts.import_lift path/to/lift_file.lift [path/to/lift_ranges.lift-ranges]')
        sys.exit(1)
    ...
    if not os.path.exists(lift_file):
        logger.error(f'LIFT file not found: {lift_file}')
        sys.exit(1)
    
    # Check if the ranges file exists if provided
    if ranges_file and not os.path.exists(ranges_file):
        logger.error(f'Ranges file not found: {ranges_file}')
        sys.exit(1)
    ...
    try:
        ...
        logger.info(f'Initializing database with LIFT file: {lift_file}')
        dict_service.initialize_database(lift_file, ranges_file)
        
        # Get entry count
        entry_count = dict_service.get_entry_count()
        logger.info(f'Database initialized with {entry_count} entries')
