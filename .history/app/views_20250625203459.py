...
def index():
    ...
    try:
        ...
        # Only try to get other stats if we're connected
...
def edit_entry(entry_id):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            logger.debug("Received data for entry update: %s", data)
            ...
            try:
                ...
                logger.error("Validation error updating entry: %s", str(e))
                return jsonify({'error': str(e)}), 400
            except Exception as e:
                logger.error("Error updating entry: %s", str(e), exc_info=True)
...
def add_entry():
    ...
    try:
        if request.method == 'POST':
            ...
            logger.debug("Received data for new entry: %s", data)
            ...
            try:
                ...
                logger.error("Error creating entry: %s", str(e), exc_info=True)
                return jsonify({'error': str(e)}), 500
...
def search():
    ...
    """
    # Get any initial query parameter
    query = request.args.get('q', '')
    
    # Get field options from request or use defaults
    fields = request.args.getlist('fields') or ['lexical_unit', 'citation_form', 'glosses', 'definitions']
    
    # Get available fields for the UI
    field_options = [
        {'id': 'lexical_unit', 'label': 'Lexical Units'},
        {'id': 'citation_form', 'label': 'Citation Forms'},
    ...
    try:
        ...
        if status.get('last_backup'):
            try:
                ...
                # BaseX returns ISO format, so we'll convert it to a more readable format
                from datetime import datetime
                timestamp = datetime.fromisoformat(status['last_backup'].replace('Z', '+00:00'))
                status['last_backup'] = timestamp.strftime('%Y-%m-%d %H:%M')
            except Exception:
                # If formatting fails, just use the original string
                pass
...
def api_search():
    ...
    try:
        ...
        # Support both pagination styles (offset-based and page-based)
        offset = request.args.get('offset')
        page = request.args.get('page')
        
        if offset is not None:
            # Use direct offset if provided
            offset = int(offset)
        ...
        logger.debug("Search API called with query: %s, fields: %s, limit: %s, offset: %s", 
        ...
        try:
            ...
            logger.debug("Search returned %s results out of %s total", len(entries), total)
        except Exception as e:
            logger.error("Error in search_entries: %s", str(e), exc_info=True)
...
def api_entries():
    """
    Get all entries or search for entries.
    """
    try:
        # Get dictionary service
        dict_service = injector.get(DictionaryService)
        ...
        if query:
            # Search for entries if a query is provided
            entries, total = dict_service.search_entries(
                query=query,
                fields=fields,
                limit=limit,
            ...
            # List all entries if no query is provided
...
@main_bp.route('/api/entry/update/<entry_id>', methods=['POST'])
def api_update_entry(entry_id):
    ...
    try:
        ...
        logger.debug("Received data for entry update API: %s", data)
        
        # Ensure ID matches
        if data.get('id') != entry_id:
            return jsonify({'error': 'Entry ID in URL does not match ID in data'}), 400
        ...
        logger.error("Entry not found: %s", str(e))
