    ...
    def search_entries(self, 
        ...
        try:
            ...
            total_count = int(self.db_connector.execute_query(count_query))
              # Search entries
            query = f"""
            for $entry at $pos in /lift/entry
            where {search_condition}
            ...
            if limit is not None:
                if offset is not None and offset > 0:
                    query += f" return subsequence($entry, {offset + 1}, {limit})"
                else:
                    query += f" return subsequence($entry, 1, {limit})"
            else:
                query += " return $entry"
