...
class DictionaryService:
    ...
    def initialize_database(self, lift_path: str, ranges_path: Optional[str] = None) -> None:
        ...
        try:
            ...
            if not db_name:
                raise DatabaseError("Database name is not configured.")
    ...
    def create_entry(self, entry: Entry) -> str:
        ...
        try:
            ...
            def strip_namespace_from_element(element: ET.Element) -> None:
    ...
    def search_entries(self, 
        ...
        try:
            ...
            # Build pagination expressions
            pagination_expr = ""
            if limit is not None and offset and offset > 0:
                pagination_expr = f"[position() > {offset} and position() <= {offset + limit}]"
            elif limit is not None:
                pagination_expr = f"[position() <= {limit}]"
    ...
    def get_entry_count(self) -> int:
        ...
        try:
            ...
            self.logger.error("Error getting entry count: %s", str(e), exc_info=True)
            raise DatabaseError(f"Failed to get entry count: {e}") from e
    ...
    def get_entries_by_grammatical_info(self, grammatical_info: str) -> List[Entry]:
        ...
        try:
            ...
            query = f"""
            xquery for $entry in doc('{db_name}')/*[local-name()='lift']/*[local-name()='entry']/*[local-name()='grammatical-info'][@value="{grammatical_info}"]/..
            return $entry
            """
