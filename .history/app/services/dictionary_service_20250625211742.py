...
class DictionaryService:
    ...
    def search_entries(self, 
        ...
        try:
            ...
            # First get the total count - adjusted for BaseX XQuery 4.0 syntax
            count_query = f"""
            xquery
            count(
            ...
            # Now get the actual entries with proper BaseX XQuery 4.0 syntax
            search_query = f"""
            xquery
            for $entry in collection('{db_name}')/lift/entry
            where {search_condition}
            ...
            if limit is not None:
                # For pagination, use position() directly in a predicate
                paginated_query = f"""
                xquery
                for $entry in collection('{db_name}')/lift/entry
                where {search_condition}
                ...
                # Add the position predicate directly as part of the XQuery
            ...
            # Add query details if they were defined - use locals().get() to avoid unbound variable errors
            local_vars = locals()
            count_query_var = local_vars.get('count_query')
            if count_query_var:
                formatted_count_query = "\n".join([line.strip() for line in count_query_var.split("\n") if line.strip()])
                error_msg += f"\n\nCount query:\n{formatted_count_query}"
                
            paginated_query_var = local_vars.get('paginated_query')
            if paginated_query_var:
