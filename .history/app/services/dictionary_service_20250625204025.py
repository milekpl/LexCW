...
class DictionaryService:
    ...
    def __init__(self, db_connector: Union[BaseXConnector, MockDatabaseConnector]):
        ...
        # Make sure the connector is connected
        if not self.db_connector.is_connected():
            try:
                self.db_connector.connect()
                self.logger.info("Connected to BaseX server in DictionaryService initialization")
            except Exception as e:
                self.logger.error("Failed to connect to BaseX server in DictionaryService initialization: %s", e, exc_info=True)
    ...
    def _prepare_entry_xml(self, entry: Entry) -> str:
        ...
        Args:
            entry: The Entry object to convert to XML.
            
        Returns:
            XML string representation of the entry.
            
        Raises:
            ValueError: If XML generation fails.
        """
        try:
            # Validate the entry before generating XML
            entry.validate()
            
            # Generate LIFT XML
            entry_xml_full = self.lift_parser.generate_lift_string([entry])
            self.logger.debug("Generated LIFT XML: %s", entry_xml_full[:500] + '...' if len(entry_xml_full) > 500 else entry_xml_full)
            ...
            if entry_elem_ns is None:
                self.logger.error("Failed to find entry element in generated XML: %s", entry_xml_full)
            ...
            result = ET.tostring(entry_elem_ns, encoding='unicode')
            self.logger.debug("Prepared entry XML: %s", result[:500] + '...' if len(result) > 500 else result)
            return result
            
        except ET.ParseError as e:
            self.logger.error("XML parsing error in _prepare_entry_xml: %s", str(e))
            raise ValueError(f"Failed to parse XML: {str(e)}")
        except Exception as e:
            self.logger.error("Error in _prepare_entry_xml: %s", str(e), exc_info=True)
            raise ValueError(f"Failed to prepare entry XML: {str(e)}")
    ...
    def search_entries(self, 
        ...
        """
        if not fields:
            fields = ["lexical_unit", "citation_form", "glosses", "definitions"]
        ...
        try:
            ...
            if "lexical_unit" in fields:
                conditions.append(f'contains(lower-case($entry/lexical-unit/form/text/text()), "{q_lower}")')
            if "citation_form" in fields:
                conditions.append(f'contains(lower-case($entry/citation/form/text/text()), "{q_lower}")')
            if "glosses" in fields:
                conditions.append(f'some $gloss in $entry/sense/gloss satisfies contains(lower-case($gloss/text/text()), "{q_lower}")')
            if "definitions" in fields or "definition" in fields:
                conditions.append(f'some $def in $entry/sense/definition/form satisfies contains(lower-case($def/text/text()), "{q_lower}")')
            
            search_condition = " or ".join(conditions)
            
            # Log the query and conditions for debugging
            self.logger.debug("Search query: %s, Fields: %s", query, fields)
            self.logger.debug("Search conditions: %s", search_condition)
            
            # Default values for pagination
            offset_val = offset if offset is not None else 0
            limit_val = limit if limit is not None else 20
            
            # Create a single-line query without line breaks to avoid XQuery syntax issues
            count_query = f"xquery count(for $entry in collection('{db_name}')/lift/entry where {search_condition} return $entry)"
            
            self.logger.debug("Executing count query (exact): %s", repr(count_query))
            
            try:
                count_result = self.db_connector.execute_query(count_query)
                total_count = int(count_result) if count_result else 0
                self.logger.debug("Search count result: %s", total_count)
            except Exception as e:
                self.logger.error("Error executing count query: %s", str(e), exc_info=True)
                # Include the count query in error message
                raise DatabaseError(f"Failed to execute count query: {str(e)}. Query was: {repr(count_query)}") from e
            
            # Use a simpler one-line query
            query_str = "xquery for $entry in collection('dictionary')/lift/entry return $entry"
            
            self.logger.debug("Executing basic search query: %s", query_str)
            
            try:
                # Include the query in debug logs for easier troubleshooting
                self.logger.debug("Executing BaseX query (exact): %s", repr(query_str))
                
                result = self.db_connector.execute_query(query_str)
                self.logger.debug("Basic search result length: %s", len(result) if result else 0)
                ...
                self.logger.error("Error executing basic search query: %s", str(e), exc_info=True)
                # Include the query in the error message for user visibility
                raise DatabaseError(f"Failed to execute basic search query: {str(e)}. Query was: {repr(query_str)}") from e
            
            # Dead code after early return - removing these lines
            # if not result:
            #     self.logger.info("No search results found for query: %s, fields: %s", query, fields)
            #     return [], total_count
            # 
            # entries = self.lift_parser.parse_string(f"<lift>{result}</lift>")
            # 
            ...
            self.logger.error("Error searching entries: %s", str(e), exc_info=True)
            # Include search parameters in the error message
            search_params = {
                'query': query,
                'fields': fields,
                'limit': limit,
            ...
            self.logger.debug("Search parameters: %s", search_params)
            
            # Log the query if it was constructed
            if 'query_str' in locals():
                self.logger.debug("Query that failed: %s", query_str)
                
            if 'count_query' in locals():
                self.logger.debug("Count query that failed: %s", count_query)
                
            # Include all the details in the error message
            error_msg = f"Failed to search entries: {str(e)}. "
            error_msg += f"Search parameters: {search_params}. "
            
            if 'count_query' in locals():
                error_msg += f"Count query: {repr(count_query)}. "
                
            if 'query_str' in locals():
                error_msg += f"Search query: {repr(query_str)}."
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        Returns:
            ...
            - last_backup: Timestamp of the last backup (if available)
            - storage_percent: Estimated storage usage percentage
        """
        try:
            status = {
                'db_connected': self.db_connector.is_connected(),
                'db_name': self.db_connector.database or 'Not configured',
                'entry_count': 0,
                'last_backup': None,
                'storage_percent': 0
            }
            
            # Only try to get these if we're connected
            if status['db_connected']:
                try:
                    # Try to count entries
                    status['entry_count'] = self.count_entries()
                except Exception as e:
                    self.logger.warning(f"Failed to count entries for system status: {e}")
                
                try:
                    # Check when the database was last modified
                    query = f"xquery db:info('{self.db_connector.database}')/timestamp/string()"
                    result = self.db_connector.execute_query(query)
                    if result:
                        status['last_backup'] = result
                except Exception as e:
                    self.logger.warning(f"Failed to get database info for system status: {e}")
                
                try:
                    # Get storage info
                    query = f"xquery db:info('{self.db_connector.database}')/size"
                    result = self.db_connector.execute_query(query)
                    if result:
                        # Rough estimate for demo purposes
                        size_mb = int(result) / (1024 * 1024)  # Convert to MB
                        # Pretend we have 100MB max storage
                        status['storage_percent'] = min(int(size_mb), 100) 
                except Exception as e:
                    self.logger.warning(f"Failed to get storage info for system status: {e}")
