    ...
    def search_entries(self, 
        ...
        try:
            ...
            let $paginated_matches := 
              ...
              then subsequence($matches, {offset + 1 if offset is not None else 1}, {limit if limit is not None else 'count($matches)'})
              else $matches
            ...
            # The result might be in a structured format or XML
            if not result:
                return [], 0
            
            # Try to parse the entries from the result
            try:
                # If the result is in JSON-like format from XQuery map
                if result.strip().startswith('{') and 'entries' in result:
                    self.logger.debug("Received JSON-like result from XQuery map")
                    
                    # Extract the entries XML part using string manipulation since we can't parse it as JSON
                    entries_start = result.find('"entries":') + len('"entries":')
                    entries_end = result.rfind('"count"')
                    if entries_end == -1:  # If count comes before entries
                        entries_end = result.rfind('}')
                    
                    entries_xml = result[entries_start:entries_end].strip().strip(',')
                    # Clean any remaining JSON syntax
                    entries_xml = entries_xml.strip('[]{}').strip()
                    
                    # Extract the count
                    count_start = result.find('"count":') + len('"count":')
                    count_end = result.find(',', count_start)
                    if count_end == -1:
                        count_end = result.rfind('}')
                    
                    try:
                        total_count = int(result[count_start:count_end].strip())
                    except (ValueError, TypeError):
                        total_count = 0
                        
                    # If we have entries, parse them
                    if entries_xml:
                ...
                # If the result is XML, try to parse it normally
                # No need to create a root element, just parse directly
                entries = self.lift_parser.parse_string(f"<lift>{result}</lift>")
                return entries, len(entries)
                    
            except Exception as e:
                # If parsing fails, log it and use the fallback approach
                self.logger.warning(f"Could not parse search result, falling back to traditional approach: {e}")
