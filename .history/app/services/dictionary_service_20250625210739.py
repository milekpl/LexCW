    ...
    def search_entries(self, 
        ...
        try:
            ...
            if not search_condition:
                # If no valid fields were selected, return nothing
                self.logger.warning("No valid search fields selected")
                return [], 0
            ...
            # First get the total count - use a simplified query format to reduce potential issues
            count_query = f"""
            xquery
            let $matching_entries :=
                for $entry in collection('{db_name}')/lift/entry
                where {search_condition}
                return $entry
            return count($matching_entries)
            """
            
            self.logger.debug("Executing count query: %s", count_query)
            
            try:
                count_result = self.db_connector.execute_query(count_query)
                total_count = int(count_result) if count_result else 0
                self.logger.debug("Total matching entries: %s", total_count)
            except Exception as e:
                self.logger.error("Error executing count query: %s", str(e), exc_info=True)
                # Format the query to be more readable in the error message
                formatted_query = "\n".join([line.strip() for line in count_query.split("\n") if line.strip()])
                error_msg = f"Failed to execute count query: {str(e)}\n\nFULL QUERY:\n{formatted_query}"
                raise DatabaseError(error_msg) from e
            
            # Now get the actual entries with pagination
            search_query = f"""
            xquery
            for $entry in collection('{db_name}')/lift/entry
            where {search_condition}
            ...
            if limit is not None:
                ...
                # Add the position predicate separately for clarity
                position_predicate = f"[position() > {offset_val} and position() <= {offset_val + limit_val}]"
                paginated_query = paginated_query.replace("return $entry", f"return $entry{position_predicate}")
            else:
                paginated_query = search_query
            
            self.logger.debug("Executing search query: %s", paginated_query)
            
            try:
                result = self.db_connector.execute_query(paginated_query)
                self.logger.debug("Search result length: %s", len(result) if result else 0)
                ...
                self.logger.error("Error executing search query: %s", str(e), exc_info=True)
                error_msg = f"Failed to execute search query: {str(e)}\nQuery: {paginated_query.replace(chr(10), ' ').strip()}"
                raise DatabaseError(error_msg) from e
            ...
            search_params = {
                ...
                'limit': limit,
                'offset': offset,
                'db_name': self.db_connector.database if hasattr(self.db_connector, 'database') else None,
            }
            
            # Don't include conditions if they're not defined yet
            if 'conditions' in locals():
                search_params['conditions'] = conditions
                
            error_msg = f"Failed to search entries: {str(e)}. Search parameters: {search_params}"
            
            # Add query details if available
            if 'count_query' in locals():
                error_msg += f" Count query: {count_query.replace(chr(10), ' ').strip()}"
                
            if 'paginated_query' in locals():
                error_msg += f" Search query: {paginated_query.replace(chr(10), ' ').strip()}"
