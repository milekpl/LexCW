...
class DictionaryService:
    ...
    def get_ranges(self) -> Dict[str, Any]:
        ...
        try:
            ...
            query = "xquery /*[local-name()='lift-ranges']"
            
            result = self.db_connector.execute_query(query)
            
            if result:
                self.ranges = self.ranges_parser.parse_string(result)
            else:
                self.ranges = {"ranges": {}}
    ...
    def get_range_values(self, range_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            range_data = ranges.get("ranges", {}).get(range_id)
            
            if range_data and "values" in range_data:
                return range_data["values"]
            
            raise NotFoundError(f"Range '{range_id}' not found or has no values.")
            
        except Exception as e:
            self.logger.error("Error getting range values for '%s': %s", range_id, str(e))
            raise DatabaseError(f"Failed to get range values: {e}") from e
