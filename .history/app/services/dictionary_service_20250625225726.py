    ...
    def search_entries(self, 
        ...
        try:
            ...
            let $matches := (
              ...
              let $match := {{
                "lexical_unit": {f"contains(lower-case(string-join($entry/lexical-unit/form/text/text(), '')), $query_lower)" if "lexical_unit" in fields else "false()"},
                "glosses": {f"some $gloss in $entry/sense/gloss/text satisfies contains(lower-case($gloss), $query_lower)" if "glosses" in fields else "false()"},
                "definitions": {f"some $def in $entry/sense/definition/form/text satisfies contains(lower-case($def), $query_lower)" if "definitions" in fields else "false()"}
              }}
            ...
            # The result might be in XML format, need to parse it
            if not result or "entries" not in result:
                return [], 0
            
            # Parse the root element which contains the entries
            try:
                root = ET.fromstring(result)
                entries_xml = root.find('.//entries')
                count_elem = root.find('.//count')
                
                if entries_xml is not None:
                    entries_text = ET.tostring(entries_xml, encoding='unicode')
                    entries = self.lift_parser.parse_string(f"<lift>{entries_text}</lift>")
                ...
                # If the result isn't valid XML, fall back to traditional approach
                self.logger.warning("Could not parse search result as XML, falling back to traditional approach")
