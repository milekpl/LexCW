...
class DictionaryService:
    ...
    def __init__(self, db_connector: Union[BaseXConnector, MockDatabaseConnector]):
        ...
        try:
            ...
            if db_name and db_name not in (self.db_connector.execute_query("LIST") or ""):
                self.logger.warning(
    ...
    def get_entry(self, entry_id: str) -> Entry:
        ...
        try:
            ...
            query = f"""
            xquery for $entry in doc('{db_name}')/*[local-name()='lift']/*[local-name()='entry'][@id="{entry_id}"]
            return $entry
            """
    ...
    def create_entry(self, entry: Entry) -> str:
        ...
        try:
            ...
            query = f"""
            xquery insert node {entry_xml} into doc('{db_name}')/*[local-name()='lift']
            """
    ...
    def update_entry(self, entry: Entry) -> None:
        ...
        try:
            ...
            query = f"""
            xquery replace node doc('{db_name}')/*[local-name()='lift']/*[local-name()='entry'][@id="{entry.id}"] with {entry_xml}
            """
    ...
    def delete_entry(self, entry_id: str) -> None:
        ...
        try:
            ...
            query = f"""
            xquery delete node doc('{db_name}')/*[local-name()='lift']/*[local-name()='entry'][@id="{entry_id}"]
            """
    ...
    def list_entries(self, limit: Optional[int] = None, offset: int = 0, sort_by: str = "lexical_unit") -> Tuple[List[Entry], int]:
        ...
        try:
            ...
            query = f"""
            xquery (for $entry in doc('{db_name}')/*[local-name()='lift']/*[local-name()='entry']
            order by {sort_expr}
    ...
    def search_entries(self, 
        ...
        try:
            ...
            count_query = f"""
            xquery count(for $entry in doc('{db_name}')/*[local-name()='lift']/*[local-name()='entry']
    ...
    def get_related_entries(self, entry_id: str, relation_type: Optional[str] = None) -> List[Entry]:
        ...
        try:
            ...
            query = f"""
            xquery let $entry_relations := doc('{db_name}')/*[local-name()='lift']/*[local-name()='entry'][@id="{entry_id}"]/relation{relation_condition}/@ref
            for $related in doc('{db_name}')/*[local-name()='lift']/*[local-name()='entry'][@id = $entry_relations]
            return $related
            """
    ...
    def count_entries(self) -> int:
        ...
        try:
            ...
            query = f"xquery count(doc('{db_name}')//*:entry)"
            result = self.db_connector.execute_query(query)
    ...
    def get_ranges(self) -> Dict[str, Any]:
        ...
        try:
            ...
            ranges_xml = self.db_connector.execute_query(f"xquery doc('{db_name}/ranges.xml')")
    ...
    def get_range_values(self, range_id: str) -> List[Dict[str, str]]:
        """
        Get the values for a specific range ID.

        Args:
            range_id: The ID of the range to get values for (e.g., "semantic-domain-ddp4").

        Returns:
            A list of value dictionaries, where each dictionary has 'id', 'guid', and 'label'.
        """
        try:
            ...
            range_data = all_ranges.get(range_id)
            if not range_data or 'range-elements' not in range_data:
                return []

            return range_data['range-elements']
        except Exception as e:
            self.logger.error("Error getting range values for %s: %s", range_id, str(e), exc_info=True)
            return []
