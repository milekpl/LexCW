...
class DictionaryService:
    ...
    def export_lift(self) -> str:
        """
        Generate a LIFT file string.
        
        Returns:
            String containing the LIFT XML content.
        ...
        try:
            ...
            self.logger.info(f"Generated LIFT XML ({len(entries)} entries)")
            return lift_xml
            
        except Exception as e:
            self.logger.error(f"Error generating LIFT XML: {e}")
            raise ExportError(f"Failed to generate LIFT XML: {e}")
    
    def export_to_lift(self, output_path: str) -> None:
        """
        Export the entire dictionary to a LIFT file.
        
        Args:
            output_path: Path to the output LIFT file.
            
        Raises:
            DatabaseError: If there is an error exporting the dictionary.
        """
        try:
            # Get all entries
            entries, total_count = self.list_entries(limit=100000)  # Assuming the dictionary is not too large
            
            # Generate LIFT file
            self.lift_parser.generate_lift_file(entries, output_path)
            
            self.logger.info(f"Dictionary exported to {output_path} ({total_count} entries)")
            
        except Exception as e:
            self.logger.error(f"Error exporting dictionary to LIFT: {e}")
            raise DatabaseError(f"Failed to export dictionary: {e}")
    
    def import_from_lift(self, input_path: str, clear_existing: bool = False) -> int:
        """
        Import entries from a LIFT file.
        
        Args:
            input_path: Path to the input LIFT file.
            clear_existing: Whether to clear existing entries before import.
        ...
        Raises:
            FileNotFoundError: If the LIFT file does not exist.
            DatabaseError: If there is an error importing entries.
        """
        try:
            # Parse the LIFT file
            entries = self.lift_parser.parse_file(input_path)
            
            # Clear existing entries if requested
            if clear_existing:
                self.db_connector.execute_command("DELETE /lift/entry")
            ...
            for entry in entries:
                try:                    # Check if entry already exists
                    ...
                    self.logger.warning(f"Error importing entry {entry.id}: {e}")
            
            return len(entries)
            
        except Exception as e:
            self.logger.error(f"Error importing from LIFT: {e}")
            raise DatabaseError(f"Failed to import from LIFT: {e}")
    ...
    def get_ranges(self) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary of ranges data.
            
        Raises:
            DatabaseError: If there is an error getting ranges data.
        """
        if self.ranges:
            return self.ranges
        ...
        try:
            ...
            import json
            self.ranges = json.loads(result)
            
            return self.ranges
            
        except Exception as e:
            self.logger.error(f"Error getting ranges data: {e}")
            raise DatabaseError(f"Failed to get ranges data: {e}")
    ...
    def get_range_values(self, range_id: str) -> List[Dict[str, Any]]:
        ...
        Raises:
            ...
            DatabaseError: If there is an error getting range values.
        """
        ...
        return ranges[range_id].get('values', [])
