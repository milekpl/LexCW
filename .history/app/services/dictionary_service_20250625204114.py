...
class DictionaryService:
    ...
    def search_entries(self, 
        ...
        try:
            ...
            # Try a very simple query first to check if the API works at all
            try:
                # Create a very basic query without complex conditions
                simple_query = "xquery count(collection('dictionary')/lift/entry)"
                
                self.logger.debug("Executing simple test query: %s", simple_query)
                count_result = self.db_connector.execute_query(simple_query)
                total_count_all = int(count_result) if count_result else 0
                self.logger.debug("Total entries: %s", total_count_all)
                
                # Now try retrieving the actual entries
                entries_query = "xquery for $entry in collection('dictionary')/lift/entry return $entry"
                
                self.logger.debug("Executing basic retrieval query: %s", entries_query)
                result = self.db_connector.execute_query(entries_query)
                
                if not result:
                    self.logger.info("No entries found in database")
                    return [], 0
                    
                entries = self.lift_parser.parse_string(f"<lift>{result}</lift>")
                # For testing purposes, return all entries but indicate in total_count the unfiltered count
                return entries, total_count_all
                
            except Exception as e:
                self.logger.error("Error executing basic database query: %s", str(e), exc_info=True)
                raise DatabaseError(f"Failed to execute basic database query: {str(e)}. This suggests there might be a fundamental connection or database structure issue.") from e
