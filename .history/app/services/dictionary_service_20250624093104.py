...
class DictionaryService:
    ...
    def initialize_database(self, lift_path: str, ranges_path: Optional[str] = None) -> None:
        ...
        try:
            ...
            # Store LIFT data in the database            self.db_connector.execute_update(f"OPEN {db_name}")
            self.db_connector.execute_update("DELETE /*")  # Clear existing data
            self.db_connector.execute_update("ADD to lift.xml", lift_data)
    ...
    def get_range_values(self, range_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if range_id in ranges.get("ranges", {}):
                return ranges["ranges"][range_id]
