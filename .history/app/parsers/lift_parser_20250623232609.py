...
class LIFTParser:
    ...
    def _parse_entry(self, entry_elem: ET.Element) -> Entry:
        ...
        if not entry_id:
          ...
          # Parse lexical unit
        lexical_unit = {}
        lexical_unit_elem = entry_elem.find('.//lift:lexical-unit', self.NSMAP)
        if lexical_unit_elem is not None:
            for form_elem in lexical_unit_elem.findall(self.XPATH_FORM, self.NSMAP):
        ...
        for citation_elem in entry_elem.findall('.//lift:citation', self.NSMAP):
            citation = {}
            for form_elem in citation_elem.findall('.//lift:form', self.NSMAP):
                lang = form_elem.get('lang')
                text_elem = form_elem.find('.//lift:text', self.NSMAP)
                if lang and text_elem is not None and text_elem.text:
        ...
        pronunciations = {}
        for pron_elem in entry_elem.findall('.//lift:pronunciation', self.NSMAP):
        ...
        for variant_elem in entry_elem.findall('.//lift:variant', self.NSMAP):
            ...
            for form_elem in variant_elem.findall('.//lift:form', self.NSMAP):
                lang = form_elem.get('lang')
                text_elem = form_elem.find('.//lift:text', self.NSMAP)
                if lang and text_elem is not None and text_elem.text:
        ...
        # Parse senses
        senses = []
        for sense_elem in entry_elem.findall('.//lift:sense', self.NSMAP):
    ...
    def _parse_sense(self, sense_elem: ET.Element, sense_id: Optional[str] = None) -> Sense:
        ...
        # Parse glosses
        glosses = {}
        for gloss_elem in sense_elem.findall('.//lift:gloss', self.NSMAP):
            lang = gloss_elem.get('lang')
            text_elem = gloss_elem.find('.//lift:text', self.NSMAP)
            if lang and text_elem is not None and text_elem.text:
                glosses[lang] = text_elem.text
        
        # Parse definitions
        definitions = {}
        for def_elem in sense_elem.findall('.//lift:definition', self.NSMAP):
            for form_elem in def_elem.findall('.//lift:form', self.NSMAP):
                lang = form_elem.get('lang')
                text_elem = form_elem.find('.//lift:text', self.NSMAP)
                if lang and text_elem is not None and text_elem.text:
    ...
    def _parse_example(self, example_elem: ET.Element, example_id: Optional[str] = None) -> Example:
        ...
        # Parse forms
        forms = {}
        for form_elem in example_elem.findall('.//lift:form', self.NSMAP):
            lang = form_elem.get('lang')
            text_elem = form_elem.find('.//lift:text', self.NSMAP)
            if lang and text_elem is not None and text_elem.text:
        ...
        for trans_elem in example_elem.findall('.//lift:translation', self.NSMAP):
            for form_elem in trans_elem.findall('.//lift:form', self.NSMAP):
                lang = form_elem.get('lang')
                text_elem = form_elem.find('.//lift:text', self.NSMAP)
                if lang and text_elem is not None and text_elem.text:
    ...
    def _generate_entry_element(self, parent: ET.Element, entry: Entry) -> ET.Element:
        ...
        if entry.lexical_unit:
            ...
            for lang, text in entry.lexical_unit.items():
                form = ET.SubElement(lex_unit, '{' + self.NSMAP['lift'] + '}form')
                form.set('lang', lang)
                text_elem = ET.SubElement(form, '{' + self.NSMAP['lift'] + '}text')
                text_elem.text = text
        ...
        for citation in entry.citations:
            ...
            for lang, text in citation.items():
                form = ET.SubElement(citation_elem, '{' + self.NSMAP['lift'] + '}form')
                form.set('lang', lang)
                text_elem = ET.SubElement(form, '{' + self.NSMAP['lift'] + '}text')
                text_elem.text = text
        ...
        for variant in entry.variant_forms:
            ...
            for lang, text in variant.items():
                if lang != 'type':
                    form = ET.SubElement(variant_elem, '{' + self.NSMAP['lift'] + '}form')
                    form.set('lang', lang)
                    text_elem = ET.SubElement(form, '{' + self.NSMAP['lift'] + '}text')
                    text_elem.text = text
        ...
        for field_type, field_value in entry.custom_fields.items():
            ...
            form = ET.SubElement(field_elem, '{' + self.NSMAP['lift'] + '}form')
            text_elem = ET.SubElement(form, '{' + self.NSMAP['lift'] + '}text')
        ...
        for sense_dict in entry.senses:
            ...
            for lang, text in sense.glosses.items():
                ...
                text_elem = ET.SubElement(gloss_elem, '{' + self.NSMAP['lift'] + '}text')
                text_elem.text = text
            ...
            if sense.definitions:
                ...
                for lang, text in sense.definitions.items():
                    form = ET.SubElement(def_elem, '{' + self.NSMAP['lift'] + '}form')
                    form.set('lang', lang)
                    text_elem = ET.SubElement(form, '{' + self.NSMAP['lift'] + '}text')
                    text_elem.text = text
            ...
            for example_dict in sense.examples:
                ...
                for lang, text in example.forms.items():
                    form = ET.SubElement(example_elem, '{' + self.NSMAP['lift'] + '}form')
                    form.set('lang', lang)
                    text_elem = ET.SubElement(form, '{' + self.NSMAP['lift'] + '}text')
                    text_elem.text = text
                ...
                if example.translations:
                    ...
                    for lang, text in example.translations.items():
                        form = ET.SubElement(trans_elem, '{' + self.NSMAP['lift'] + '}form')
                        form.set('lang', lang)
                        text_elem = ET.SubElement(form, '{' + self.NSMAP['lift'] + '}text')
                        text_elem.text = text
...
class LIFTRangesParser:
    ...
    def _parse_range(self, range_elem: ET.Element) -> Dict[str, Any]:
        ...
        # Parse range labels
        for label_elem in range_elem.findall('.//lift:label', self.NSMAP):
            lang = label_elem.get('lang')
            if lang and label_elem.text:
                range_data['description'][lang] = label_elem.text
        
        # Parse range values
        for value_elem in range_elem.findall('.//lift:range-element', self.NSMAP):
            value = {
            ...
            # Parse value labels
            for label_elem in value_elem.findall('.//lift:label', self.NSMAP):
                lang = label_elem.get('lang')
                if lang and label_elem.text:
                    value['description'][lang] = label_elem.text
            
            # Parse child values (for hierarchical ranges)
            for child_elem in value_elem.findall('.//lift:range-element', self.NSMAP):
    ...
    def _parse_range_element(self, elem: ET.Element) -> Dict[str, Any]:
        ...
        for label_elem in elem.findall('.//lift:label', self.NSMAP):
            lang = label_elem.get('lang')
            if lang and label_elem.text:
                element_data['description'][lang] = label_elem.text
        
        # Parse child elements (recursive)
        for child_elem in elem.findall('.//lift:range-element', self.NSMAP):
