...
class LIFTParser:
    ...
    def _parse_entry(self, entry_elem: ET.Element) -> Entry:
        ...
        return entry    def _parse_sense(self, sense_elem: ET.Element, sense_id: Optional[str] = None) -> Sense:
        """
        ...
        for gloss_elem in sense_elem.findall('.//lift:gloss', self.NSMAP):
            ...
            text_elem = gloss_elem.find(self.XPATH_TEXT, self.NSMAP)
            if lang and text_elem is not None and text_elem.text:
        ...
        for def_elem in sense_elem.findall('.//lift:definition', self.NSMAP):
            for form_elem in def_elem.findall(self.XPATH_FORM, self.NSMAP):
                lang = form_elem.get('lang')
                text_elem = form_elem.find(self.XPATH_TEXT, self.NSMAP)
                if lang and text_elem is not None and text_elem.text:
