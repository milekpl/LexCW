...
class LIFTRangesParser:
    ...
    def _parse_range(self, range_elem: ET.Element) -> Dict[str, Any]:
        ...
        range_data = {
            ...
            'description': {}        }
          # Parse range labels
        for label_elem in self._find_elements(range_elem, self.XPATH_LABEL, './/label'):
            lang = label_elem.get('lang')
            if lang and label_elem.text:
                range_data['description'][lang] = label_elem.text
        
        # Parse range values
        for value_elem in self._find_elements(range_elem, self.XPATH_RANGE_ELEMENT, './/range-element'):
            value = {
              ...
              # Parse value labels
            for label_elem in self._find_elements(value_elem, self.XPATH_LABEL, './/label'):
                lang = label_elem.get('lang')
                if lang and label_elem.text:
                    value['description'][lang] = label_elem.text
            
            # Parse child values (for hierarchical ranges)
            for child_elem in self._find_elements(value_elem, self.XPATH_RANGE_ELEMENT, './/range-element'):
    ...
    def _parse_range_element(self, elem: ET.Element) -> Dict[str, Any]:
        ...
        for label_elem in self._find_elements(elem, self.XPATH_LABEL, './/label'):
            lang = label_elem.get('lang')
            if lang and label_elem.text:
                element_data['description'][lang] = label_elem.text
        
        # Parse child elements (recursive)
        for child_elem in self._find_elements(elem, self.XPATH_RANGE_ELEMENT, './/range-element'):
