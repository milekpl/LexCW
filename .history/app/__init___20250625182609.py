...
def configure_dependencies(binder):
    ...
    binder.bind(BaseXConnector, to=BaseXConnector(
        host=os.getenv('BASEX_HOST', 'localhost'),
        port=int(os.getenv('BASEX_PORT', '1984')),
        username=os.getenv('BASEX_USERNAME', 'admin'),
        password=os.getenv('BASEX_PASSWORD', 'admin'),
        database=os.getenv('BASEX_DATABASE', 'sample-lift-file')
    ), scope=singleton)

    def provide_dictionary_service(connector: BaseXConnector) -> DictionaryService:
        """Provides a DictionaryService instance."""
        return DictionaryService(db_connector=connector)

    binder.bind(DictionaryService, to=provide_dictionary_service, scope=singleton)
