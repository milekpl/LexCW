...
class SQLiteExporter(BaseExporter):
    ...
    def export(self, output_path: str, entries: Optional[List[Entry]] = None, 
               source_lang: str = "en", target_lang: str = "pl", batch_size: int = 500) -> str:
        """
        ...
        try:
            ...
            try:
                ...
                for i in range(0, total_entries, batch_size):
                    ...
                    self.logger.info(f"Processed {min(i+batch_size, total_entries)}/{total_entries} entries")
            ...
            self.logger.info(f"SQLite database exported to {output_path}")
            return output_path
            
        except Exception as e:
            self.logger.error(f"Error exporting to SQLite format: {e}")
            raise ExportError(f"Failed to export to SQLite format: {e}")
