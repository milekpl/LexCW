...
// Import the serialization functions with cache-busting to ensure fresh code
self.importScripts("/static/js/form-serializer.js?v=" + Date.now());
...
self.addEventListener("message", function (e) {
  try {
    ...
    try {
      const count = Array.isArray(formData)
        ? formData.length
        : (formData && formData.length) || 0;
      const sampleNames = (Array.isArray(formData)
        ? formData.slice(0, 10)
        : formData
      ).map
        ? Array.isArray(formData)
          ? formData.slice(0, 10).map((e) => e[0])
          : []
        : [];
