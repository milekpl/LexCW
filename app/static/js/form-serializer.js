...
function serializeFormToJSON(input, options = {}) {
  ...
  if (
    ...
    const realSenses = input.querySelectorAll(
    ...
    throw new Error(
      "Input must be an HTMLFormElement, FormData object, or FormData-like object"
    );
  }
  ...
  multiValueFields.forEach((values, key) => {
    ...
    if (fieldCount % 100 === 0) {
      console.debug(
        `[FormSerializer] Processed ${fieldCount} fields, last: ${key}`
      );
    }

    // Determine the value to set: single value, array (legitimate multi-value), or coerced scalar.
    let processedValue;
    if (values.length === 1) {
      processedValue = values[0];
    } else {
      processedValue = coerceMultiValueField(input, key, values);
    }
  ...
  if (
    ...
    window.FormSerializerProblemFields &&
    window.FormSerializerProblemFields.length > 0
  ) {
    console.warn(
      `[FormSerializer] Skipped ${window.FormSerializerProblemFields.length} problematic fields. See window.FormSerializerProblemFields for details.`
    );
  }
  console.debug(
    `[FormSerializer] Finished processing ${fieldCount} fields. Last field: ${lastField}`
  );
  return result;
}
...
function coerceMultiValueField(formOrFormData, key, values) {
  // For non-form inputs (FormData passed directly or worker/test doubles), we can't inspect element types.
  // Use conservative heuristics: preserve arrays only for known multi-value fields, otherwise coerce.
  if (!formOrFormData || typeof formOrFormData.querySelectorAll !== "function") {
    const keyStr = String(key);
    const isExplicitArray = keyStr.endsWith("[]") || keyStr.includes("[]");
    const isKnownMultiSelect = /(\.|^)(domain_type|semantic_domain|usage_type)(\.|$)/.test(
      keyStr
    );

    if (isExplicitArray || isKnownMultiSelect) {
      return values;
    }
    ...
    // Prefer the last non-empty value as the most recent/visible control.
  ...
  const selector = `[name="${safeCssEscape(key)}"]`;
  const elements = Array.from(formOrFormData.querySelectorAll(selector));

  const isMultipleSelect = elements.some(
    (el) =>
      el &&
      el.tagName &&
  ...
  const isCheckboxGroup =
    elements.length > 1 &&
    elements.every(
      (el) => el && el.tagName && el.tagName.toLowerCase() === "input" && el.type === "checkbox"
    );
  if (isCheckboxGroup) {
    return values;
  }
  ...
  // Common duplicate-name pattern: an empty template field + the real value.
  if (nonEmptyValues.length === 1) {
    return nonEmptyValues[0];
  }

  // If duplicates are identical, keep a single scalar.
  if (nonEmptyValues.length > 1 && nonEmptyValues.every((v) => v === nonEmptyValues[0])) {
    return nonEmptyValues[0];
  }

  console.warn(
    `[FormSerializer] Warning: Field "${key}" has ${values.length} values but does not appear to be a legitimate multi-value control. Coercing to a scalar using the last non-empty value.`,
    values
  );
  if (nonEmptyValues.length > 0) {
    return nonEmptyValues[nonEmptyValues.length - 1];
...
function setNestedValue(obj, path, value, depth = 0, fieldName = null) {
  ...
  if (depth > MAX_DEPTH) {
    console.error(
      `[FormSerializer] Max depth exceeded at field '${fieldName || path}'`
    );
    throw new Error(
      `Max object depth exceeded at field '${fieldName || path}'`
    );
  }
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    if (isLast) {
      if (isArrayIndex) {
        ...
        if (index > MAX_ARRAY_SIZE) {
          console.error(
            `[FormSerializer] Array index too large (${index}) at field '${
              fieldName || path
            }'`
          );
          throw new Error(
            `Array index too large (${index}) at field '${fieldName || path}'`
          );
        }
    ...
    if (isArrayIndex) {
      ...
      if (index % 1000 === 0 && index > 0) {
        console.debug(
          `[FormSerializer] Large array index: ${index} at field '${
            fieldName || path
          }'`
        );
      }
      ...
      if (!current[key]) {
        ...
        const nextIsArrayIndex =
          i + 1 < keys.length && keys[i + 1].isArrayIndex;
    ...
    if (depth + i > 0 && (depth + i) % 10 === 0) {
      console.debug(
        `[FormSerializer] Deep recursion at field '${
          fieldName || path
        }', depth: ${depth + i}`
      );
    }
...
function parseFieldPath(path) {
  ...
  while (currentPath.length > 0) {
    ...
    if (parseStep > 100) {
      console.error(
        `[FormSerializer] parseFieldPath: Too many parse steps for path '${path}'`
      );
      throw new Error(
        `parseFieldPath: Too many parse steps for path '${path}'`
      );
    }
    ...
    if (dotIndex === -1 && bracketIndex === -1) {
      ...
      dotIndex !== -1 &&
      (bracketIndex === -1 || dotIndex < bracketIndex)
    ) {
  ...
  if (keys.length > 20) {
    console.debug(
      `[FormSerializer] parseFieldPath: Long key path (${keys.length} segments) for '${path}'`
    );
  }
  return keys;
}
...
function validateFormForSerialization(form) {
  ...
  formData.forEach((value, key) => {
    ...
    if (key.includes("..")) {
      result.warnings.push(
        `Field "${key}" contains consecutive dots which may cause issues`
      );
    }

    if (key.includes("[]") && !key.endsWith("[]")) {
      result.warnings.push(
        `Field "${key}" contains empty array notation in middle of path`
      );
    }
  ...
  const arrayFields = fieldNames.filter(
  ...
  Object.entries(arrayGroups).forEach(([arrayName, indices]) => {
    ...
    for (let i = 0; i < sortedIndices.length - 1; i++) {
      if (sortedIndices[i + 1] - sortedIndices[i] > 1) {
        result.warnings.push(
          `Array "${arrayName}" has gaps in indices: missing index ${
            sortedIndices[i] + 1
          }`
        );
      }
...
function serializeFormToJSONSafe(input, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      reject(
        new Error("Form serialization timed out. The form may be too complex.")
      );
      ...
      if (window.Worker) {
        try {
          ...
          const worker = new Worker(
            `/static/js/form-serializer-worker.js?v=${Date.now()}`
          );
            ...
            if (e.data.error) {
              ...
              try {
                if (e.data.result && !e.data.result.id) {
                  ...
                  console.warn(
                    "[FormSerializer] Worker result missing id; assigned temporary id:",
                    tempId
                  );
                }
              } catch (fixErr) {
                console.debug(
                  "[FormSerializer] Failed to assign temp id to worker result",
                  fixErr
                );
              }
          ...
          if (input instanceof HTMLFormElement) {
            ...
            const templateElement = input.querySelector(
              "#default-sense-template"
            );
            const realSenses = input.querySelectorAll(
              ".sense-item:not(#default-sense-template):not(.default-sense-template)"
            );
            ...
            if (templateElement && realSenses.length > 0) {
              ...
              const templateFields = Array.from(
                templateElement.querySelectorAll("[name]")
              ).map((f) => f.name);
              formEntries = formEntries.filter(
                ([name, value]) => !templateFields.includes(name)
              );
            } else if (templateElement && realSenses.length === 0) {
          ...
          try {
            ...
            console.debug("[FormSerializer] Posting to worker", {
              count: formEntries.length,
              sampleNames: sample,
            });
          } catch (logErr) {
            console.debug(
              "[FormSerializer] Posting to worker (could not stringify sample)",
              logErr
            );
          }
