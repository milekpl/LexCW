...
def list_entries() -> Any:
    ...
    try:
        ...
        if per_page is not None and per_page < 1:
            return jsonify(
                {"error": "Per_page parameter must be a positive integer"}
            ), 400
        ...
        for i, entry in enumerate(entries):
            try:
                ...
                logger.error(
                    f"Entry at index {i} has wrong type: {type(entry)}. Error: {e}"
                )
                raise e
        ...
        if "xml" in content_type:
            ...
            try:
                ...
                if isinstance(e, InvalidXMLError):
                    return jsonify({"error": str(e)}), 400
                if isinstance(e, DatabaseConnectionError) or isinstance(
                    e, XMLEntryServiceError
                ):
                    logger.error(
                        f"[XML API] Error creating XML entry via /api/entries: {e}"
                    )
                    return jsonify({"error": str(e)}), 500
                logger.error(
...
def update_entry(entry_id: str) -> Any:
    ...
    try:
        ...
        logger.info(
            f"[SENSE UPDATE] Number of senses in request: {len(data.get('senses', []))}"
        )
        for i, sense in enumerate(data.get("senses", [])):
            logger.info(f"[SENSE UPDATE]   Sense {i}: id={sense.get('id')}")
            logger.info(
                f"[SENSE UPDATE]   Sense {i} definition: {sense.get('definition')}"
            )
                    ...
                    for lang_key, content in sense[field].items():
                        if isinstance(content, dict):
                            ...
                            if "text" in content and content.get("text", "").strip():
                                ...
                                logger.info(
                                    f"[SENSE UPDATE] Removed empty {field} for language '{lang_key}' from sense"
                                )
        ...
        try:
            ...
            entry_elem = root.find(
                ".//{http://fieldworks.sil.org/schemas/lift/0.13}entry"
            )
        ...
        if existing_entry and existing_entry.senses:
            for i, sense in enumerate(existing_entry.senses):
                ...
                logger.info(
                    f"[SENSE UPDATE] Existing sense {i} definition: {sense.definition}"
                )
        ...
        skip_validation = (
            data.pop("skip_validation", False)
            or request.args.get("skip_validation", "false").lower() == "true"
        )
        ...
        try:
            ...
            for s in data.get("senses", []) or []:
                ...
                if has_definition or has_gloss:
                    ...
                    logger.info(
                        f"[SENSE UPDATE] Dropping empty/template sense from submission: id={s.get('id')}"
                    )
        ...
        logger.info(
            f"[SENSE UPDATE] Entry after from_dict - senses: {[{'id': s.id, 'definitions': s.definitions} for s in entry.senses]}"
        )

        logger.info(
            f"[SENSE UPDATE] New entry object has {len(entry.senses) if entry.senses else 0} senses"
        )
        ...
        project_id = session.get("project_id")
        logger.info(
            f"[SENSE UPDATE] Calling dict_service.update_entry with skip_validation={skip_validation}, project_id={project_id}"
        )
...
def clear_entries_cache() -> Any:
    ...
    try:
        ...
        if cache.is_available():
            ...
            return jsonify(
                {"status": "success", "message": "Entries cache cleared."}
            ), 200
        else:
        ...
        return jsonify(
            {"status": "error", "message": f"Error clearing cache: {e}"}
        ), 500
            ...
            },
            {
                ...
                "schema": {
                    ...
                    "properties": {
                        "target_entry_id": {
                            "type": "string",
                            "description": "ID of the target entry to create variant relation with",
                        },
                        "variant_type": {
                            "type": "string",
                            "description": "Type of variant relation (e.g., spelling, dialectal, archaic)",
                        },
                        "direction": {
                            "type": "string",
                            "enum": ["outgoing", "incoming"],
                            "description": "Direction of relation: outgoing (this entry HAS target as variant) or incoming (this entry IS a variant of target)",
                        },
                    },
                },
            },
...
def create_variant_relation(entry_id: str) -> Any:
    ...
    try:
        ...
        if not xml_service.entry_exists(target_entry_id):
            return jsonify(
                {"error": f"Target entry '{target_entry_id}' not found"}
            ), 404
        ...
        if source_entries:
            ...
            for var in existing_variants:
                if var.get("ref") == target_entry_id:
                    return jsonify(
                        {
                            "error": f"Duplicate: entry '{entry_id}' already has a variant relation to '{target_entry_id}'"
                        }
                    ), 400
        ...
        if target_entries:
            ...
            for var in target_variants:
                if var.get("ref") == entry_id:
                    return jsonify(
                        {
                            "error": f"Circular reference: entry '{target_entry_id}' already has a variant relation to '{entry_id}'"
                        }
                    ), 400
        ...
        if direction == "outgoing":
            ...
            modified_xml = _add_variant_relation_to_xml(
                source_xml, target_entry_id, variant_type, extra_traits
            )
            ...
            target_xml_data = xml_service.get_entry(target_entry_id)
            target_xml = target_xml_data["xml"]
            modified_target_xml = _add_variant_relation_to_xml(
                target_xml, entry_id, variant_type, extra_traits
            )
...
def _add_variant_relation_to_xml(
    xml_string: str, target_id: str, variant_type: str, extra_traits: dict = None
) -> str:
    """
    ...
    if entry_elem is None:
        ...
        if (
            root_tag == "entry"
            or root_tag == "{http://fieldworks.sil.org/schemas/lift/0.13}entry"
        ):
            entry_elem = root
            ns = {}
    ...
    for rel in entry_elem.findall(
        ".//{http://fieldworks.sil.org/schemas/lift/0.13}relation", ns
    ):
        if rel.get("ref") == target_id:
            # Check if it has variant-type trait
            for trait in rel.findall(
                ".//{http://fieldworks.sil.org/schemas/lift/0.13}trait", ns
            ):
                if trait.get("name") == "variant-type":
                    raise ValueError(
                        f"Variant relation to '{target_id}' already exists"
                    )
    ...
    ET.SubElement(
        relation_elem,
        f"{relation_ns}trait",
        {"name": "variant-type", "value": variant_type},
    )
...
def get_variant_relations(entry_id: str) -> Any:
    ...
    try:
        ...
        return jsonify({"entry_id": entry_id, "variants": all_variants})
...
@entries_bp.route(
    "/<string:entry_id>/variants/<string:target_entry_id>", methods=["DELETE"]
)
@swag_from(
    {
        "tags": ["Entries"],
...
def delete_variant_relation(entry_id: str, target_entry_id: str) -> Any:
    ...
    try:
        ...
        modified_xml, source_modified = _remove_variant_relation_from_xml(
            source_xml, target_entry_id
        )
        ...
        if not source_modified:
            ...
            if xml_service.entry_exists(target_entry_id):
                ...
                modified_target_xml, target_modified = (
                    _remove_variant_relation_from_xml(target_xml, entry_id)
                )
                if target_modified:
                    xml_service.update_entry(target_entry_id, modified_target_xml)
                    return jsonify(
            ...
            return jsonify(
                {
                    "error": f"No variant relation found between '{entry_id}' and '{target_entry_id}'"
                }
            ), 404
