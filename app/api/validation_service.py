...
# NOTE: Batch validation for `/api/validation/batch` is implemented by
# `validate_batch()` in `app/api/validation.py` (which also supports the
# `validate_all` full-dictionary audit and relation-target checks). This
# blueprint intentionally does NOT register a colliding `/api/validation/batch`
# route to avoid ambiguous routing between the two handlers.
