...
class DictionaryService:
    ...
    def __init__(self, 
        ...
        self._non_validating_parser = LIFTParser(validate=False)
    ...
    def _ensure_db_configured(self) -> str:
        """Ensure database is configured and return db_name."""
        db_name = self.db_connector.database
        if not db_name:
            raise DatabaseError(DB_NAME_NOT_CONFIGURED)
        return db_name

    def _get_namespace_context(self) -> Tuple[str, str, str, str, str, str, str, str, str]:
        """
        Get namespace-aware context for queries.
        
        Returns:
            Tuple of (prologue, entry_path, lexical_unit_path, form_path, text_path, 
                     citation_path, sense_path, grammatical_info_path, gloss_path)
        """
        ...
        return (prologue, entry_path, lexical_unit_path, form_path, text_path, 
                citation_path, sense_path, grammatical_info_path, gloss_path, definition_path)
    ...
    def get_entry(self, entry_id: str) -> Entry:
        ...
        """
        try:
            db_name = self._ensure_db_configured()
            ...
            if (
                ...
                self.logger.debug(f"Returning hardcoded test entry: {entry.id}")
                return entry
            ...
            self.logger.debug(f"Executing query for entry: {entry_id}")
            self.logger.debug(f"Query: {query}")
            entry_xml = self.db_connector.execute_query(query)

            if not entry_xml:
                self.logger.debug(f"Entry {entry_id} not found in database {db_name}")
            ...
            self.logger.debug(f"Entry XML: {entry_xml[:100]}...")
            entries = self.lift_parser.parse_string(entry_xml)
            if not entries or not entries[0]:
                self.logger.debug(f"Error parsing entry {entry_id}")
                raise NotFoundError(f"Entry with ID '{entry_id}' could not be parsed")

            entry = entries[0]
            self.logger.debug(f"Entry parsed successfully: {entry.id}")
    ...
    def list_entries(
        ...
        try:
            ...
            if not result:
                return [], total_count
            # Use cached non-validating parser
            entries = self._non_validating_parser.parse_string(f"<lift>{result}</lift>")
    ...
    def import_lift(self, lift_path: str, mode: str = "merge") -> int:
        ...
        Args:
            lift_path: Path to the LIFT file.
            mode: Import mode - "replace" (drop existing DB and create new) or "merge" (update existing)
        ...
        if mode == "replace":
            return self._import_lift_replace(lift_path)
        else:
            return self._import_lift_merge(lift_path)

    def _import_lift_replace(self, lift_path: str) -> int:
        """
        Replace the entire database with the LIFT file content.
        This drops the existing database and creates a new one from the LIFT file.
        """
        try:
            if not os.path.exists(lift_path):
                raise FileNotFoundError(f"LIFT file not found: {lift_path}")
            ...
            lift_path_basex = os.path.abspath(lift_path).replace("\\", "/")
            self.logger.info("Replacing database with LIFT file: %s", lift_path_basex)
            ...
            try:
                ...
                self.logger.warning("Error dropping existing database: %s", e)
            
            # Create new database from LIFT file
            self.logger.info("Creating new database from LIFT file")
            ...
            if os.path.exists(ranges_path):
                ranges_path_basex = os.path.abspath(ranges_path).replace("\\", "/")
                self.logger.info("Adding ranges file to database: %s", ranges_path_basex)
                self.db_connector.execute_command(f'OPEN {db_name}')
                self.db_connector.execute_command(f'ADD "{ranges_path_basex}"')
            
            # Count entries in the new database
            # LIFT files always have namespaces conceptually, but may not use prefixes
            # We'll detect the actual namespace usage in the newly created database
            has_ns = self._detect_namespace_usage_in_db(db_name)
            entry_path = self._query_builder.get_element_path("entry", has_ns)
            prologue = self._query_builder.get_namespace_prologue(has_ns)
            
            count_query = f"{prologue} count(collection('{db_name}')//{entry_path})"
            total_count = int(self.db_connector.execute_query(count_query) or 0)
            
            self.logger.info("Database replaced with %d entries from LIFT file", total_count)
            return total_count

        except Exception as e:
            self.logger.error("Error replacing database with LIFT file: %s", str(e), exc_info=True)
            raise DatabaseError(f"Failed to replace database with LIFT file: {e}") from e

    def _import_lift_merge(self, lift_path: str) -> int:
        """
        Merge entries from a LIFT file into the existing database using fast bulk operations.
        """
        try:
            if not os.path.exists(lift_path):
                raise FileNotFoundError(f"LIFT file not found: {lift_path}")
            ...
            try:
                ...
                # Check for associated .lift-ranges file and add it to temp database
                ranges_path = lift_path.replace('.lift', '.lift-ranges')
                if os.path.exists(ranges_path):
                    ranges_path_basex = os.path.abspath(ranges_path).replace("\\", "/")
                    self.logger.info("Adding ranges file: %s", ranges_path_basex)
                    self.db_connector.execute_command(f'ADD {temp_db_name} "{ranges_path_basex}"')
                
                # Verify temp database was created
                db_list = self.db_connector.execute_command("LIST") or ""
                if temp_db_name not in db_list:
                    raise DatabaseError(f"Failed to create temp database {temp_db_name}")
                
                # LIFT files may or may not have namespaces
                ...
                main_lift_path_elem = self._query_builder.get_element_path("lift", main_has_ns)
                ...
                # Fast bulk merge using XQuery - copy entries directly
                # First, delete existing entries with matching IDs
                self.logger.info("Removing existing entries with matching IDs...")
                delete_query = f"""
                {prologue}
                let $source_ids := collection('{temp_db_name}')//{temp_entry_path}/@id/string()
                for $id in $source_ids
                let $existing := collection('{self.db_connector.database}')//{main_entry_path}[@id = $id]
                return if (exists($existing)) then delete node $existing else ()
                """
                self.db_connector.execute_query(delete_query)
                
                # Then insert all source entries
                self.logger.info("Inserting new entries...")
                temp_lift_path = self._query_builder.get_element_path("lift", temp_has_ns)
                insert_query = f"""
                {prologue}
                let $source_entries := collection('{temp_db_name}')//{temp_entry_path}
                let $target_lift := collection('{self.db_connector.database}')//{main_lift_path_elem}
                return if (exists($target_lift)) then
                  insert node $source_entries into $target_lift
                else
                  for $node in collection('{temp_db_name}')/*
                  return insert node $node into collection('{self.db_connector.database}')
                """
                self.db_connector.execute_query(insert_query)
                
                # Count entries in main database after insert to verify success
                final_count_query = f"{prologue} count(collection('{self.db_connector.database}')//{main_entry_path})"
                final_count = int(self.db_connector.execute_query(final_count_query) or 0)
                
                self.logger.info("Successfully merged entries: %d expected, %d actual", total_count, final_count)
                
                if final_count == 0 and total_count > 0:
                    self.logger.warning("Import may have failed - expected %d entries but found %d", total_count, final_count)
                ...
                try:
                    db_list = self.db_connector.execute_command("LIST") or ""
                    if temp_db_name in db_list:
                        self.db_connector.execute_command(f"DROP DB {temp_db_name}")
                        self.logger.info("Temp database cleaned up")
                except Exception as cleanup_error:
                    self.logger.warning("Error during temp database cleanup: %s", cleanup_error)
    ...
    def export_lift(self, project_id: int = 1, dual_file: bool = False) -> str:
        ...
        try:
            ...
            if dual_file:
                # Generate base filename for ranges reference
                import datetime
                timestamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
                base_filename = f"dictionary_export_{timestamp}"
                lift_xml = self._convert_to_dual_file_format(lift_xml, base_filename)
    ...
    def _convert_to_dual_file_format(self, lift_xml: str, base_filename: str) -> str:
        """
        ...
        Args:
            ...
            base_filename: Base filename for generating ranges file reference
        ...
        try:
            ...
            for range_elem in ranges_elem.findall('range'):
                ...
                if range_id:
                    ...
                    # Use the actual ranges filename that will be generated
                    new_range_elem.set('href', f'{base_filename}.lift-ranges')
